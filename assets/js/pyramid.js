(function ($) {
  $.fn.pyramid = function (options) {
    ...
    clickStep = function () {
      ...
      _thisFeedback
        .addClass("visible")
        .after('<div class="popupFeedbackShadow"></div>');
    };
        ...
        for (h = settings.stepLength - 1; h >= 0; h--) {
          step
            ...
            .css({
              ...
              width:
                (widthValue / settings.stepLength) * (settings.stepLength - h),
              "margin-left":
                (widthValue / (settings.stepLength * 2)) * settings.stepLength
            })
            .addClass("step" + (settings.stepLength - h));

          step
            .eq(h - (settings.stepLength - 1))
            .find("span")
            .css({
